...
| `datafusion` | `Scan::declarative_metadata_scan_plan`, compiled and executed by `DataFusionExecutor` |
...
The no-checkpoint table exercises JSON replay plus `Aggregate(max_non_null_by(...))`; DataFusion
lowers that to an ordered aggregate without a window sort. For a checkpoint at the latest version,
Kernel's plan optimizer removes JSON replay, aggregation, and the checkpoint anti-join, leaving a
Parquet scan, filter, and projections. The DV table adds composite reconciliation keys.
...
Set `VERSION=50` in the matrix environment, or pass `--version 50` to the binary, to benchmark
an exact time-travel snapshot.
