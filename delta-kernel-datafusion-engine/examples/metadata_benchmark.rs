...
fn usage() -> &'static str {
    "Usage: metadata_benchmark --method METHOD [OPTIONS]\n\
     ...
     Options:\n\
       ...
       --version N        Time-travel to an exact Delta table version\n\
...
async fn explain_datafusion_plan(
    ...
    let Some(dataframe) = executor.scan_metadata(scan)? else {
        eprintln!("Metadata plan is empty");
        return Ok(());
    };
    eprintln!(
        "Logical plan:\n{}",
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut snapshot_builder = Snapshot::builder_for(table_url);
