//! Shared helpers for the streaming [`super::LoadExec`] and the load dispatch consumed by
//! the executor driver in a downstream module of this workspace slice.
...
/// Name of the parquet `_row_number` virtual column appended by [`build_file_source`] when
/// `include_row_number` is set. Matches the symbol DataFusion 54 will install via
/// `TableSchema::with_virtual_columns` (apache/datafusion#22026). The DV-driven filter in
/// [`build_per_file_plan`] reads back this column by name.
...
/// and the full passthrough values vector (every entry in `node.passthrough_columns`).
/// `partition_values` is always the full passthrough length to match the
/// `TableSchema`'s declared partition fields; DataFusion's source pruning drops the
/// unprojected ones at scan time.
#[derive(Debug, Clone)]
...
/// Extract one upstream row into a [`RowInputs`]. `projected_passthrough` is the
/// precomputed set of `node.passthrough_columns` indices to materialize; per `LoadExec::new`
/// this is always the full `0..passthrough_count` range. Kept as a parameter so the
/// helper stays decoupled from `LoadExec`'s field layout for testing.
pub(crate) fn extract_row_inputs(
    ...
    // Path columns are always Utf8 in scan-emitted Load plans, but we validate the array
    // type rather than blindly downcasting so an upstream change that emits LargeUtf8
    // produces a typed error instead of a panic.
    let path_str = match path_arr.data_type() {
        ArrowDataType::Utf8 => path_arr.as_string::<i32>().value(row),
        other => {
            return Err(internal_error(format!(
                "LoadNode path column `{path_cn}` has unexpected arrow type {other:?}; \
                 expected Utf8"
            )));
        }
    };
    let url = resolve_file_location(node, path_str)?;
...
pub(crate) fn build_file_source(
    ...
    // Top-level metadata stays so a future field-id-aware `PhysicalExprAdapter` can match
    // by `PARQUET:field_id` / `delta.columnMapping.*` (required for ID-mode column
    // mapping). `StampFieldUdf` re-stamps logical nested metadata at the SSA `Project`
    // boundary.
    //
    ...
    // `_row_number` virtual column requires DataFusion 54's
    // `TableSchema::with_virtual_columns` (apache/datafusion#22026). Until that ships,
    // callers must not request row-number; the LoadExec wrapper rejects DV plans up
    // front so this path is unreachable.
    if include_row_number {
        return Err(datafusion_common::error::DataFusionError::NotImplemented(
            "_row_number virtual column requires DataFusion 54".to_string(),
        ));
    }
...
/// `RecordBatch::try_new` does full `DataType` equality including metadata baked into
/// nested child fields, but the top-level field's own metadata is part of the `Field`,
/// not the `DataType`, and is not subject to that equality check. So we only need to
/// strip the child metadata to satisfy `RecordBatch::try_new`, while keeping top-level
/// keys like `PARQUET:field_id` and `delta.columnMapping.*` so the future field-id-aware
/// `PhysicalExprAdapter` can still match columns by id (critical for ID-mode column
/// mapping where logical names don't uniquely identify the physical column).
...
/// Parquet decoding will switch to field-id/column-mapping aware adaptation once the
/// kernel field-id adapter lands (DataFusion 54 ships the supporting
/// `with_virtual_columns` / `PhysicalExprAdapter` plumbing); JSON keeps DataFusion's
/// default name-based adapter. Until then this returns `None` for both file types and
/// [`super::LoadExec::new`] rejects column-mapping plans up front so silent name-based
...
pub(crate) async fn build_per_file_plan(
    ...
    let Some(dv) = dv else {
        // Schema mismatch here means the upstream LoadNode declared a different output
        // shape than `DataSourceExec` materialized -- a contract violation that must
        // surface in release builds too, not just under `debug_assertions`. Without this
        // check, a release-mode mismatch returns a plan with the wrong shape and
        // downstream operators see silently-misaligned columns.
        let plan_n = plan.schema().fields().len();
        let out_n = output_schema.fields().len();
        if plan_n != out_n {
            return Err(internal_error(format!(
                "build_per_file_plan: DataSourceExec schema mismatch with output_schema \
                 (no DV); DataSourceExec has {plan_n} fields, output_schema has {out_n}"
            )));
        }
        return Ok(plan);
    };
