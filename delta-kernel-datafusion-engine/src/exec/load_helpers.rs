...
/// Resolve a dynamic scan's `base_url`. Scan-emitted plans always set it; the runtime check
/// only catches IR-level misuse.
pub(crate) fn load_base_url(node: &DynamicScan) -> Result<&Url, DataFusionError> {
    Ok(&node.base_url)
}
...
pub(crate) fn resolve_file_location(
    ...
    base.join(path_str.trim()).map_err(|e| {
        plan_compilation(format!(
            "DynamicScan could not join base_url `{base}` with path `{path_str}`: {e}"
        ))
    })
}
...
pub(crate) fn extract_row_inputs(
    ...
    let size_arr = extract_column_array(batch, &node.file_size_column)?;
    let size = size_arr.as_primitive::<Int64Type>().value(row);
    let dv_descriptor = read_optional_dv_descriptor(batch, node, row)?;
    let partition_values = projected_passthrough
        .iter()
        .map(|&i| {
            let name = &node.file_constant_columns[i];
            let arr = batch.column_by_name(name).cloned().ok_or_else(|| {
                plan_compilation(format!(
                    "dynamic scan input is missing file constant `{name}`"
                ))
            })?;
...
fn read_optional_dv_descriptor(
    ...
    let sa = arr.as_struct_opt().ok_or_else(|| {
        internal_error(format!(
            "DynamicScan dv_ref column `{}` must be a struct matching DeletionVectorDescriptor",
            dv_cn
        ))
    })?;
    let col = |name: &str| {
...
fn strip_field_metadata_recursive(field: &ArrowField) -> ArrowField {
    ...
    ArrowField::new(field.name(), stripped_dt, true)
}
...
pub(crate) fn strip_nested_metadata_only(field: &ArrowField) -> ArrowField {
    let stripped_dt = strip_nested_metadata_dt(field.data_type());
    ArrowField::new(field.name(), stripped_dt, true).with_metadata(field.metadata().clone())
}
...
fn strip_nested_metadata_dt(dt: &ArrowDataType) -> ArrowDataType {
    ...
    match dt {
        ...
        ArrowDataType::Map(entry, sorted) => {
            let entry_type = match entry.data_type() {
                ArrowDataType::Struct(children) => ArrowDataType::Struct(
                    children
                        .iter()
                        .enumerate()
                        .map(|(index, child)| {
                            let data_type = strip_nested_metadata_dt(child.data_type());
                            Arc::new(ArrowField::new(child.name(), data_type, index != 0))
                        })
                        .collect(),
                ),
                other => strip_nested_metadata_dt(other),
            };
            ArrowDataType::Map(
                Arc::new(ArrowField::new(entry.name(), entry_type, false)),
                *sorted,
            )
        }
        other => other.clone(),
