//! Lazy [`TableProvider`] for [`DynamicScan`]: defers all work to `scan()`, which lowers the
//! upstream `LogicalPlan` and wraps it in a [`super::LoadExec`]. Filter pushdown is currently
//! off; projection and limit flow through to [`super::LoadExec`].
...
use delta_kernel::plans::ir::nodes::DynamicScan;
...
/// Table provider that lowers a [`DynamicScan`] upstream into [`LoadExec`] at scan time.
pub struct LoadTableProvider {
    upstream_logical: LogicalPlan,
    node: Arc<DynamicScan>,
...
impl LoadTableProvider {
    /// Construct from a dynamic scan and its upstream file relation.
    pub fn try_new(
        upstream_logical: LogicalPlan,
        node: Arc<DynamicScan>,
        engine: Arc<dyn Engine>,
        ...
        let file_field_count = node.schema.fields().len() - node.file_constant_columns.len();
        let kernel_arrow_schema: ArrowSchema = node
            .schema
            .as_ref()
            .try_into_arrow()
