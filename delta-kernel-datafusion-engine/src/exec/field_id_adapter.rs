...
//! [`DataType`]s differ -- wraps the column in `CastExpr` to reconcile nested fields and apply
//! type widening.
...
use delta_kernel::arrow::datatypes::{Field, Fields, SchemaRef};
...
mod tests {
    ...
    fn no_field_ids_passthrough() -> DfResult<()> {
        ...
        assert!(rewritten.as_any().downcast_ref::<CastExpr>().is_none());
