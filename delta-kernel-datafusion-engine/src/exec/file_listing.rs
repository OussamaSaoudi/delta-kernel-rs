...
impl ExecutionPlan for FileListingExec {
    ...
    fn execute(
        ...
        // Build a scheme/authority-only origin URL for the object-store registry lookup.
        // Using `scheme://host` alone drops the port and breaks custom endpoints
        // (e.g. MinIO at `http://127.0.0.1:9000/...`). `Url::origin()` preserves the
        // port; fall back to the path itself for schemes without a host (e.g. opaque
        // schemes used by in-memory stores).
        let mut origin = self.path.clone();
        origin.set_path("/");
        origin.set_query(None);
        origin.set_fragment(None);
        let object_store_url = ObjectStoreUrl::parse(origin.as_str())?;
