...
impl LoadExec {
    /// Build a load plan over `upstream` using the dynamic scan and optional projection.
    pub fn new(
        upstream: Arc<dyn ExecutionPlan>,
        node: Arc<DynamicScan>,
        engine: Arc<dyn Engine>,
        ...
        let file_count = node.schema.fields().len() - node.file_constant_columns.len();
        let passthrough_count = node.file_constant_columns.len();
        ...
        let file_source_with_dv = (node.file_type == FileType::Parquet)
            .then(|| {
                build_file_source(
                    node.file_type,
                    &full_schema,
