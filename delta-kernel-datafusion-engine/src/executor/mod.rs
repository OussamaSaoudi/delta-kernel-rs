//! [`DataFusionExecutor`]: drives kernel coroutine state machines and compiles their step
...
use delta_kernel::plans::state_machines::framework::state_machine::{
    EngineRequest, EngineResponse, NextStep, SchemaQuery,
};
...
impl DataFusionExecutor {
    ...
    /// Compile a [`ResultPlan`] to a [`DataFrame`]. Useful for callers that already hold a
    ...
    ///
    /// Sugar for `self.drive_to_dataframe(scan.scan_state_machine()?)`.
    ...
    ///
    /// Sugar for `self.drive_to_dataframe(scan.scan_metadata_state_machine()?)`.
    ...
    ///
    /// Sugar for `self.drive_to_dataframe(fsr.state_machine()?)`.
    ...
    pub async fn execute_step(&self, op: EngineRequest) -> Result<EngineResponse, EngineError> {
        self.run_phase(op).await
    }

    /// Execute one [`EngineRequest`].
