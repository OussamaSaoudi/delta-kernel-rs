...
fn generate_primitive_extract(
    ...
    let udf = match prim {
        ...
        // Delta encodes binary stats as base64 in JSON; producing the raw bytes requires
        // a base64-decode step not yet wired through the IR. Returning the base64 string
        // here would silently corrupt skipping comparisons, so reject explicitly.
        PrimitiveType::Binary => {
            return Err(unsupported(format!(
                "json_parse: Binary column extraction at path {path:?} requires base64-decode \
                 support not yet wired up in the DataFusion engine"
            )));
        }
    };
