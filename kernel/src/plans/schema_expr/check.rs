...
fn synthesize(expr: &Expression, input_schema: &StructType) -> SchemaExprResult<(DataType, bool)> {
    Ok(match expr {
        ...
        Expression::If(if_expr) => {
            let (t_ty, t_n) = synthesize(&if_expr.then_expr, input_schema)?;
            let (e_ty, e_n) = synthesize(&if_expr.else_expr, input_schema)?;
            (unify_types([t_ty, e_ty].into_iter(), "if")?, t_n || e_n)
        }
...
/// - `Transform` / `Opaque` / `Unknown`: no type rule -- column refs validated, `expected` trusted.
///
...
mod tests {
    ...
    #[case::if_both_nullable(Expression::if_then_else(Predicate::column(["flag"]), col("ts"), col("ts")), DataType::LONG, true)]
    #[case::if_then_lit_else_col(Expression::if_then_else(Predicate::column(["flag"]), lit(0i64), col("ts")), DataType::LONG, true)]
