...
//!   [`genawaiter2::rc::Co`]. SM bodies emit yields through the plan-construction
//!   [`Context`](super::plan_context::Context)'s `reduce` / `schema_query` helpers; this module
//!   never exposes raw yields.
...
//! because the only path to `Co::yield_` is through the plan-construction
//! [`Context`](super::plan_context::Context) dispatch helpers; SM bodies are
//! `pub(crate)`-only and FFI sees them through `Box<dyn StateMachine>`, so the panic paths
//! are unreachable externally.
//!
//! # Naming note
//!
...
/// live relation snapshot) and awaits the engine outcome. The driver shuttles operations
/// to [`StateMachine::get_step`] and resumes the body with the matching `StepResume` on
