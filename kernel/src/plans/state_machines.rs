...
impl SnapshotStateMachine {
    ...
    fn create_load_metadata_phase_and_plan(&self) -> DeltaResult<(SnapshotPhase, SnapshotPlan)> {
        ...
        for commit in log_segment.listed.ascending_commit_files.iter().rev() {
            let scan = ScanNode {
                file_type: FileType::Json,
...
impl ScanBuildState {
    ...
    fn stats_schema(&self) -> Option<SchemaRef> {
        ...
        build_stats_schema(referenced_schema)
    }
...
impl ScanStateMachine {
    ...
    fn create_commit_phase_and_plan(
        ...
        use crate::scan::log_replay::{
            get_add_transform_expr, get_add_transform_expr_with_root_stats,
            scan_row_schema_with_stats_parsed, SCAN_ROW_SCHEMA,
        };
        use crate::schema::{DataType, StructField, StructType};
        ...
        // Build data skipping plan and determine output schema
        // When stats are available, include stats_parsed in output to match DefaultEngine behavior
        let (data_skipping, output_schema, transform_expr) =
            match (state.predicate(), state.stats_schema().as_ref()) {
                (Some((predicate, _)), Some(stats_schema)) => {
                    let skipping_pred =
                        as_sql_data_skipping_predicate(&predicate, &std::collections::HashSet::new());
                    let data_skipping = skipping_pred.map(|pred: crate::expressions::Predicate| DataSkippingPlan {
                        parse_json: ParseJsonNode {
                            json_column: "add.stats".to_string(),
                            target_schema: stats_schema.clone(),
                            // Output as "stats_parsed" struct column to match predicate's
                            // column references (stats_parsed.minValues.*, etc.)
                            output_column: "stats_parsed".to_string(),
                        },
                        filter: FilterByExpressionNode {
                            predicate: Arc::new(pred.into()),
                        },
                    });
                    // Include stats_parsed in output schema and transform.
                    // Use get_add_transform_expr_with_root_stats because ParseJsonNode adds
                    // stats_parsed at root level, not under add.*
                    let output_schema = scan_row_schema_with_stats_parsed(Some(stats_schema.clone()));
                    let transform_expr =
                        get_add_transform_expr_with_root_stats(Some(stats_schema.clone()), false);
                    (data_skipping, output_schema, transform_expr)
                }
                _ => {
                    // No data skipping - use base schema without stats_parsed
                    (
                        None,
                        SCAN_ROW_SCHEMA.clone(),
                        get_add_transform_expr(None, false, false, None, false),
                    )
                }
            };
    ...
    fn create_checkpoint_leaf_phase_and_plan(
        ...
        let transform_expr = get_add_transform_expr(None, false, false, None, false);
    ...
    fn create_json_checkpoint_phase_and_plan(
        ...
        let plan = JsonCheckpointPhasePlan {
            ...
            dedup_filter,
            project: SelectNode {
                columns: vec![get_add_transform_expr(None, false, false, None, false)],
