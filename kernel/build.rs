...
fn main() {
    ...
    // Generate prost bindings for the declarative-plans proto schema only when
    // the feature is enabled. Off-by-default consumers don't pay the protoc /
    // codegen cost.
    if std::env::var("CARGO_FEATURE_DECLARATIVE_PLANS").is_ok() {
