...
/// Convert ObjectMeta slice to RecordBatch with columns: path, size, modificationTime.
fn metas_to_batch(metas: &[ObjectMeta], schema: &ArrowSchemaRef) -> DfResult<RecordBatch> {
    if metas.is_empty() {
...
/// Streaming conversion of ObjectMeta items to RecordBatches.
...
/// Collects all items, sorts by path, then emits batches. Used for local filesystem.
        ...
        loop {
            match &mut self.state {
                ...
                SortedStreamState::Emitting { inner } => {
                    match ready!(inner.as_mut().poll_next(cx)) {
                        Some(chunk) => {
                            let batch = metas_to_batch(&chunk, &self.schema);
