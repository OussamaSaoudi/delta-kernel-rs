...
pub fn lower_expression(
    ...
    match expr {
        ...
        Expression::ParseJson(_) => {
            Err(DfError::ExpressionLowering(
                "ParseJson expression not yet supported".to_string()
            ))
        }
        Expression::MapToStruct(_) => {
            Err(DfError::ExpressionLowering(
                "MapToStruct expression not yet supported".to_string()
            ))
        }
...
/// Lower a Transform expression to a DataFusion named_struct expression.
...
/// Lower a kernel ColumnName to a DataFusion Expr (preserves case).
