...
/// Prepare scan files by converting FileMeta to PartitionedFile and extracting ObjectStoreUrl.
fn prepare_scan_files(files: &[FileMeta]) -> DfResult<PreparedScanFiles> {
    let first_file_url = files.first()
        .ok_or_else(|| DfError::PlanCompilation("Scan node has no files".to_string()))?
        .location.clone();

    let partitioned_files: Vec<PartitionedFile> = files.iter()
        .map(|file| PartitionedFile::new(file.location.path().to_string(), file.size as u64))
        .collect();
    ...
    Ok(PreparedScanFiles { file_group, object_store_url })
}
...
fn compile_parquet_scan(node: &ScanNode) -> DfResult<Arc<dyn ExecutionPlan>> {
    ...
    // 4. Prepare scan files (convert FileMeta to PartitionedFile and extract ObjectStoreUrl)
    ...
    let file_scan_config = FileScanConfigBuilder::new(
        prepared.object_store_url,
        file_schema,
        parquet_source,
    )
    .with_file_group(prepared.file_group) // Single group preserves file order
    ...
    // 8. Wrap with NullabilityValidationExec which:
...
fn compile_json_scan(node: &ScanNode) -> DfResult<Arc<dyn ExecutionPlan>> {
    ...
    // 2. Prepare scan files (convert FileMeta to PartitionedFile and extract ObjectStoreUrl)
    let prepared = prepare_scan_files(&node.files)?;

    // 3. Create JsonSource (newline-delimited JSON)
    let json_source = Arc::new(JsonSource::default());

    // 4. Build FileScanConfig using the builder
    let file_scan_config = FileScanConfigBuilder::new(
        prepared.object_store_url,
        file_schema,
        json_source,
    ...
    // 5. Create DataSourceExec from the config
