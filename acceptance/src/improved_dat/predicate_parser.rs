...
use sqlparser::ast::{
    BinaryOperator, Expr, FunctionArg, FunctionArgExpr, FunctionArguments, UnaryOperator, Value,
};
...
impl<'a> SchemaAwareParser<'a> {
    ...
    /// For function calls, looks inside arguments to find the first column reference.
        ...
        match expr {
            ...
            Expr::BinaryOp { left, right, .. } => {
                ...
                self.extract_column_name(left)
                    .or_else(|| self.extract_column_name(right))
            }
            _ => None,
        }
    }
    ...
    /// Converts `TIMESTAMP_NTZ'...'` and `TIMESTAMP'...'` cast syntax to plain quoted strings,
    /// relying on schema-aware type coercion to produce the correct scalar type.
