//! Shared lists and helpers for the acceptance-workload test binaries.
//!
//! Each `tests/*.rs` file is compiled as its own binary; this module lives in
//! `tests/common/mod.rs` so Cargo doesn't treat it as a standalone test binary,
//! and both `acceptance_workloads_reader` and `acceptance_workloads_datafusion`
//! pull it in via `mod common;`.
//!
//! All items are `pub` so each binary can name them; only `acceptance_workloads_reader`
//! actually uses `EXPECTED_KERNEL_FAILURES` directly while the DataFusion harness
//! references it for transformation-based expected-failure computation.

#![allow(dead_code)]

use std::sync::Arc;

use delta_kernel::schema::ColumnMappingMode;
...
/// All other failures (bugs, divergences, missing features) go in
...
/// Returns the first SKIP_LIST entry that matches `test_path`, if any.
pub fn should_skip_test(test_path: &str) -> Option<&'static str> {
    SKIP_LIST
        .iter()
        .find_map(|(pattern, reason)| test_path.contains(pattern).then_some(*reason))
}

/// Reason categories used by `datafusion_executor_unsupported_feature`. The kernel default
/// engine handles all of these today; the DataFusion executor cannot until DataFusion 54
/// ships virtual `_row_number` + field-id-aware physical adapters (see the
/// `delta-kernel-datafusion-engine` README).
pub const DF_DV_UNSUPPORTED: &str =
    "DataFusion executor does not yet support deletion vectors (requires DataFusion 54)";
pub const DF_CM_UNSUPPORTED: &str =
    "DataFusion executor does not yet support column mapping (requires DataFusion 54)";

/// Returns `Some(reason)` if the snapshot uses a table feature the DataFusion executor
/// cannot drive on DataFusion 53 -- column mapping (any non-`None` mode) or deletion
/// vectors. Callers treat a `Some` return as an expected DataFusion failure rather than
/// panicking on the `LoadExec`-level rejection.
///
/// Row tracking is not detected here because the kernel currently does not expose the
/// protocol feature list outside the crate; row-tracking workloads land in
/// `ADDITIONAL_DF_FAILURES` by spec-path infix instead.
pub fn datafusion_executor_unsupported_feature(snapshot: &Arc<Snapshot>) -> Option<&'static str> {
    if snapshot
        .table_properties()
        .column_mapping_mode
        .is_some_and(|mode| mode != ColumnMappingMode::None)
    {
        return Some(DF_CM_UNSUPPORTED);
    }
    if snapshot
        .table_properties()
