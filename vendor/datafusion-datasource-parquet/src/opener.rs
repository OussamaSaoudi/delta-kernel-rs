...
pub(super) struct ParquetOpener {
    ...
    /// Virtual Arrow columns forwarded into [`ArrowReaderOptions::with_virtual_columns`].
...
impl FileOpener for ParquetOpener {
    fn open(&self, partitioned_file: PartitionedFile) -> Result<FileOpenFuture> {
        ...
        let virtual_columns = self.virtual_columns.clone();
        ...
        Ok(Box::pin(async move {
            ...
            // Arrow virtual columns (native row numbers, etc.) extend the Arrow decode schema but do
            // not correspond to additional Parquet roots on `SchemaDescriptor`. Projection indices are
            // logical indices into the merged Arrow schema; filter down to parquet roots before
            // building the leaf mask (matches upstream discussion around DF #20133).
            //
            // When virtual columns are enabled, arrow-rs expects an unconstrained parquet projection
            // (`ProjectionMask::all`) so synthesized columns survive late projection — narrowing only to
            // referenced parquet roots drops virtual outputs from the decode stream.
            let mask = if !virtual_columns.is_empty() {
                ProjectionMask::all()
            } else {
                let parquet_root_len = builder.parquet_schema().root_schema().get_fields().len();
                let indices: Vec<usize> = projection
                    .column_indices()
                    .into_iter()
                    .filter(|&idx| idx < parquet_root_len)
                    .collect();
