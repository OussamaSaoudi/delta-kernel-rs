...
pub struct ParquetSource {
    ...
    /// Optional Arrow virtual columns merged into [`parquet::arrow::arrow_reader::ArrowReaderOptions`]
    /// (for example parquet [`RowNumber`](parquet::arrow::schema::virtual_type::RowNumber)).
    ///
    /// Default empty: identical to upstream DataFusion behavior.
...
impl ParquetSource {
    ...
    /// Attach parquet reader virtual columns (extension-type fields such as native row numbers).
    ///
    /// These are forwarded to arrow-rs [`ArrowReaderOptions::with_virtual_columns`]; validation
    /// matches upstream parquet rules (each field must be a virtual extension type).
    ///
    /// # Errors
    ///
    /// Returns if `virtual_columns` contains a non-virtual Arrow field.
    pub fn with_virtual_columns(mut self, virtual_columns: Vec<FieldRef>) -> Result<Self> {
        let _ = parquet::arrow::arrow_reader::ArrowReaderOptions::new()
